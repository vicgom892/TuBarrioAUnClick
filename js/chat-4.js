...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // === CARGA DE OFERTAS (desde seccion-ofertas.json) ===
  async function cargarOfertas() {
    try {
      const CACHE_KEY = 'ofertas_cache_v1';
      const cachedData = localStorage.getItem(CACHE_KEY);
  ...
  function addMessage(text, sender) {
    ...
    contentDiv.innerHTML = formatMessageLinks(text); // ✅ Usa innerHTML
  ...
  // === TARJETAS DE NEGOCIOS CON BOTONES DIRECTOS ===
  ...
  function generateResponse(query) {
    ...
    const tieneUbicacion = /ubicación|direccion|donde|lugar|mapa|ubicado/i.test(query);
    ...
    if (tieneNegocio && tieneHorario) {
      return "Te muestro los horarios de los comercios. Si querés, escribí el nombre de uno para ver si está abierto ahora.";
    }

    if (tieneNegocio && tieneUbicacion) {
      return "Acá tenés los comercios con su ubicación. Cada tarjeta incluye botones para contactar directamente.";
    }
    ...
    if (tieneHorario) {
      return "Acá tenés los horarios de los comercios. ¿Querés ver uno específico?";
    }

    if (tieneUbicacion) {
      return "Te muestro dónde están los comercios. ¿Buscás alguno en particular?";
    }
