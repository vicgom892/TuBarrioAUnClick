// main.js - Versión corregida y actualizada a v36
document.addEventListener('DOMContentLoaded', function() {
  ...
  const APP_VERSION = 'v37'; // ⬅️ ¡DEBE COINCIDIR EXACTAMENTE CON CACHE_VERSION EN sw.js!
  
  if ("serviceWorker" in navigator) {
    // Registrar el SW con la versión en el query string para romper la caché
      ...
      .then(registration => {
        ...
        registration.addEventListener('updatefound', () => {
          ...
          newWorker.addEventListener('statechange', () => {
            // Mostrar modal SOLO cuando el nuevo SW está "installed" y hay uno activo
            if (newWorker.state === 'installed' && navigator.serviceWorker.controller) {
              // Verificar que la versión es realmente diferente
              if (newWorker.scriptURL.includes(`v=${APP_VERSION}`)) {
                console.log('✅ Nueva versión detectada:', APP_VERSION);
                showUpdateModal(registration);
              }
            }
            
            // Cuando el nuevo SW se activa completamente
            if (newWorker.state === 'activated') {
              ...
              // Actualizar versión actual en localStorage
        ...
        navigator.serviceWorker.addEventListener('message', event => {
          if (event.data?.type === 'SW_ACTIVATED') {
            console.log('🆕 Nuevo SW activado completamente. Versión:', event.data.version);
            
            // Actualizar versión actual
            localStorage.setItem('currentVersion', event.data.version);
  ...
  function showUpdateModal(registration) {
    // Verificar si ya hemos mostrado el modal para esta versión
    const currentVersion = localStorage.getItem('currentVersion');
    if (currentVersion === APP_VERSION) {
      console.log('✅ Ya se mostró el modal para esta versión');
      return;
    }
    ...
    if (!updateModal) {
      ...
      updateModal.innerHTML = `
        <div class="update-modal-content">
          ...
          <h3>¡Hay una nueva versión!</h3>
          <p>Ya está disponible una actualización con nuevas funciones y mejoras de rendimiento.</p>
          <div class="update-actions">
            <button id="update-later" class="btn-update btn-outline">Más tarde</button>
            <button id="update-now" class="btn-update btn-primary">Actualizar ahora</button>
          </div>
        </div>
      `;
    ...
    // Asignar eventos si no existen
    if (updateNowBtn && !updateNowBtn.dataset.eventsAdded) {
      updateNowBtn.addEventListener('click', () => {
        ...
        if (registration && registration.waiting) {
          ...
          // Mostrar mensaje de actualización en progreso
          showUpdateProgressToast();
      ...
      updateNowBtn.dataset.eventsAdded = 'true';
    }
    
    if (updateLaterBtn && !updateLaterBtn.dataset.eventsAdded) {
      updateLaterBtn.addEventListener('click', () => {
        updateModal.classList.remove('active');
        console.log("🕒 Usuario pospuso la actualización.");
      });
      updateLaterBtn.dataset.eventsAdded = 'true';
    }
    
    // Cerrar modal al hacer clic fuera
    if (!updateModal.dataset.eventsAdded) {
      ...
      updateModal.dataset.eventsAdded = 'true';
    }
  ...
  // Función para mostrar toast de actualización exitosa
  function showUpdateSuccessToast() {
    // Crear toast si no existe
    let toast = document.getElementById('update-success-toast');
    if (!toast) {
      toast = document.createElement('div');
      toast.id = 'update-success-toast';
      toast.className = 'fixed top-6 left-1/2 transform -translate-x-1/2 bg-green-500 text-white px-6 py-3 rounded-full shadow-lg text-sm font-medium z-50 opacity-0 transition-all duration-300 flex items-center gap-2 max-w-xs';
      toast.innerHTML = `
        <i class="fas fa-check-circle"></i>
        <span>¡Actualización completada! Disfruta de las mejoras.</span>
      `;
  ...
  // Función para mostrar toast de actualización en progreso
  function showUpdateProgressToast() {
    let toast = document.getElementById('update-progress-toast');
    if (!toast) {
      toast = document.createElement('div');
      toast.id = 'update-progress-toast';
      toast.className = 'fixed top-6 left-1/2 transform -translate-x-1/2 bg-blue-500 text-white px-6 py-3 rounded-full shadow-lg text-sm font-medium z-50 opacity-0 transition-all duration-300 flex items-center gap-2 max-w-xs';
      toast.innerHTML = `
        <i class="fas fa-spinner fa-spin"></i>
        <span>Actualizando... Por favor espera.</span>
      `;
      document.body.appendChild(toast);
    }
    ...
    // Ocultar toast después de 5 segundos (por si la actualización tarda)
    setTimeout(() => {
      toast.classList.remove('opacity-100');
      toast.classList.add('opacity-0');
      setTimeout(() => {
  ...
  // Verificar actualizaciones al hacer clic en algún botón o acción
  ...
  function initializeFeatures() {
    ...
    window.searchBusinesses = function() {
      ...
      // Usar el índice para búsquedas rápidas
      const results = searchBusinesses(normalizedSearch);
      const openBusinesses = results.filter(b => isBusinessOpen(b.hours));
      if (openBusinesses.length > 0) {
        modalBody.innerHTML = openBusinesses.map(b => `
          <div class="card mb-3">
            <div class="card-body">
              <h5 class="card-title">${b.name}</h5>
              <p class="card-text">Categoría: ${b.category}</p>
              <p class="card-text">Horario: ${b.hours}</p>
              <a href="${b.url || '#'}" target="_blank" class="btn btn-primary">Visitar</a>
            </div>
          </div>
        `).join("");
        bootstrapModal.show();
      } else {
        modalBody.innerHTML = "<p>No se encontraron negocios abiertos ahora.</p>";
        bootstrapModal.show();
      }
    };
